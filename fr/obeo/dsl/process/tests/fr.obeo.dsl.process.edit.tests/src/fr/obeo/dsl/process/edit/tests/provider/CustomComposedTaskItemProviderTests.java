...
/**
 * Tests {@link fr.obeo.dsl.process.provider.CustomComposedTaskItemProvider CustomComposedTaskItemProvider}.
 * 
 * @author <a href="mailto:yvan.lussaud@obeo.fr">Yvan Lussaud</a>
 */
public class CustomComposedTaskItemProviderTests {
	...
	/**
	 * Tests the initial tasks choice of value.
	 */
	@Test
	public void initialTasksChoiceOfValue() {
		final ComposedTask composedTask = ProcessPackage.eINSTANCE.getProcessFactory().createComposedTask();
		composedTask.setName("composedTask");
		final ComposedTask task1 = ProcessPackage.eINSTANCE.getProcessFactory().createComposedTask();
		task1.setName("task1");
		...
		final IItemPropertySource itemPropertySource = (IItemPropertySource)factory.adapt(composedTask,
				IItemPropertySource.class);
		final IItemPropertyDescriptor descriptor = itemPropertySource.getPropertyDescriptor(composedTask,
				ProcessPackage.eINSTANCE.getComposedTask_InitialTasks());

		Collection<?> values = descriptor.getChoiceOfValues(composedTask);
		assertEquals(0, values.size());

		composedTask.getTasks().add(task1);

		values = descriptor.getChoiceOfValues(composedTask);
		assertEquals(1, values.size());
		assertEquals(task1, values.iterator().next());
	...
	/**
	 * Tests the final tasks choice of value.
	 */
	@Test
	public void finalTasksChoiceOfValue() {
		final ComposedTask composedTask = ProcessPackage.eINSTANCE.getProcessFactory().createComposedTask();
		composedTask.setName("composedTask");
		final ComposedTask task1 = ProcessPackage.eINSTANCE.getProcessFactory().createComposedTask();
		task1.setName("task1");
		...
		final IItemPropertySource itemPropertySource = (IItemPropertySource)factory.adapt(composedTask,
				IItemPropertySource.class);
		final IItemPropertyDescriptor descriptor = itemPropertySource.getPropertyDescriptor(composedTask,
				ProcessPackage.eINSTANCE.getComposedTask_FinalTasks());
	...
	/**
	 * Tests {@link fr.obeo.dsl.process.provider.CustomComposedTaskItemProvider#getImage(Object)}.
	 */
	@Test
	public void getImage() {
		...
		assertTrue(image.toString().endsWith("icons/full/obj16/ComposedTask.gif"));
		...
		assertTrue(image instanceof URL);
		assertTrue(image.toString().endsWith("icons/full/obj16/ComposedTask_initial.gif"));
		...
		image = provider.getImage(task1);
		assertTrue(image instanceof URL);
		assertTrue(image.toString().endsWith("icons/full/obj16/ComposedTask_final.gif"));
	...
	/**
	 * Tests {@link fr.obeo.dsl.process.provider.CustomComposedTaskItemProvider#getText(Object)}.
	 */
	@Test
	public void getText() {
		final ComposedTask composedTask = ProcessPackage.eINSTANCE.getProcessFactory().createComposedTask();
		...
		assertEquals("Composed Task task1", text);

		composedTask.getTasks().add(task1);
		composedTask.getInitialTasks().add(task1);

		text = provider.getText(task1);
		assertEquals("Composed Task task1 (initial)", text);

		composedTask.getInitialTasks().remove(task1);
		composedTask.getFinalTasks().add(task1);

		text = provider.getText(task1);
		assertEquals("Composed Task task1 (final)", text);
