...
/**
 * Edit {@link Task#getPrecondition() precondition}.
 * 
...
public class EditPrecondition implements IExternalJavaAction2 {
	...
	public void execute(Collection<? extends EObject> selections, Map<String, Object> parameters) {
		Task task = (Task)selections.iterator().next();
		final ExpressionDialog dialog = new ExpressionDialog(PlatformUI.getWorkbench().getDisplay()
				.getActiveShell(), task, ProcessPackage.eINSTANCE.getTask_Precondition());
		dialog.open();
	}
	...
	public boolean canExecute(Collection<? extends EObject> selections) {
		return selections.size() == 1 && selections.iterator().next() instanceof Task;
	}
