...
import org.gemoc.execution.engine.feedback.data.impl.easy.BooleanAndReferenceFeedbackData;
import org.gemoc.execution.engine.feedback.policy.FeedbackPolicy;
import org.gemoc.execution.engine.feedback.policy.impl.easy.exceptions.CorrespondingDomainSpecificEventNotFoundException;
 ...
 * @author flatombe
 */
public class EasyGoingFeedbackPolicy implements
		FeedbackPolicy<BooleanAndReferenceFeedbackData> {
	 ...
	 */
	private DomainSpecificEvent findEventWhereTargetIs(EObject eobject,
	 ...
	 */
	@Override
	public void processFeedback(BooleanAndReferenceFeedbackData feedbackData,
			Solver solver, List<DomainSpecificEvent> possibleEvents) {
		for (Boolean key : feedbackData.getMap().keySet()) {
			for (EObject eobject : feedbackData.getMap().get(key)) {
				try {
					DomainSpecificEvent eventToForce = this
							.findEventWhereTargetIs(eobject, possibleEvents);
					if (key) {
