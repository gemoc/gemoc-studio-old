...
//							}
//							Activator.getMessagingSystem().debug(
//									"Is the second object an EOperation ?: " + (linkedOperation instanceof EOperation),
//									Activator.PLUGIN_ID);
...
//		}
	...
	private EObject getEObjectFromReference(Reference reference){
		Activator.getMessagingSystem().debug("Trying to retrieve the EObject from the Reference : " + reference.toString(), Activator.PLUGIN_ID);
		...
		if(reference instanceof ModelElementReference){
			...
			throw new RuntimeException("Context reference is neither a ModelElementReference nor a NamedElementReference");
		}
		return null;
	}
