...
public class Kermeta3EventExecutor extends BasicEventExecutor {
	...
	public Kermeta3EventExecutor(ClassLoader classLoader,
			String bundleSymbolicName, IDSAExecutor dsaExecutor) {
	...
	public FeedbackData execute(ModelSpecificAction msa) {
		...
		try {
			Activator.getMessagingSystem().debug(msa.getTarget().toString(),
					Activator.PLUGIN_ID);
			Activator.getMessagingSystem().debug(
					msa.getOperation().getName().toString(),
					Activator.PLUGIN_ID);
			Activator.getMessagingSystem().debug(parameters.toString(),
					Activator.PLUGIN_ID);
