...
public class TfsmTraceExplorer implements ITraceExplorer {
	...
	private List<List<? extends tfsmTrace.States.Value>> getAllValueTraces() {
		...
		for (tfsmTrace.States.tfsm.TracedTimeFSM tracedObject : traceRoot.getTfsm_tracedTimeFSMs()) {
	...
	private void goTo(EObject eObject) {
		if (eObject instanceof tfsmTrace.States.State) {
			...
			for (tfsmTrace.States.TimeFSM_currentState_Value value : stateToGo.getTimeFSM_currentState_Values()) {
				if (value.getParent() instanceof tfsmTrace.States.tfsm.TracedTimeFSM) {
					tfsmTrace.States.tfsm.TracedTimeFSM parent_cast = (tfsmTrace.States.tfsm.TracedTimeFSM) value
							.getParent();
					org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.State toset = (org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.State) getTracedToExe(
							value.getCurrentState());
					org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.State current = ((org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.TimeFSM) parent_cast
							.getOriginalObject()).getCurrentState();
					if (current != toset) {
						((org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.TimeFSM) parent_cast.getOriginalObject())
								.setCurrentState((org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.State) toset);
					}
				}
			}
			for (tfsmTrace.States.FSMEvent_isTriggered_Value value : stateToGo.getFSMEvent_isTriggered_Values()) {
				if (value.getParent() instanceof tfsmTrace.States.tfsm.TracedFSMEvent) {
					...
					boolean current = ((org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.FSMEvent) parent_cast
							.getOriginalObject()).isIsTriggered();
					if (current != toset) {
						((org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.FSMEvent) parent_cast.getOriginalObject())
								.setIsTriggered((boolean) toset);
					}
				}
			}
			for (tfsmTrace.States.TimeFSM_lastStateChangeStepNumber_Value value : stateToGo
					.getTimeFSM_lastStateChangeStepNumber_Values()) {
				if (value.getParent() instanceof tfsmTrace.States.tfsm.TracedTimeFSM) {
					tfsmTrace.States.tfsm.TracedTimeFSM parent_cast = (tfsmTrace.States.tfsm.TracedTimeFSM) value
							.getParent();
					int toset = value.getLastStateChangeStepNumber();
					int current = ((org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.TimeFSM) parent_cast
			...
			for (tfsmTrace.States.FSMClock_numberOfTicks_Value value : stateToGo.getFSMClock_numberOfTicks_Values()) {
				if (value.getParent() instanceof tfsmTrace.States.tfsm.TracedFSMClock) {
					...
					java.lang.Integer current = ((org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.FSMClock) parent_cast
							.getOriginalObject()).getNumberOfTicks();
					if (current != toset) {
						((org.gemoc.sample.legacytfsm.xstfsm.xstfsm.tfsm.FSMClock) parent_cast.getOriginalObject())
								.setNumberOfTicks((java.lang.Integer) toset);
					}
				}
			}
			for (tfsmTrace.States.TimeFSM_stepNumber_Value value : stateToGo.getTimeFSM_stepNumber_Values()) {
				if (value.getParent() instanceof tfsmTrace.States.tfsm.TracedTimeFSM) {
					tfsmTrace.States.tfsm.TracedTimeFSM parent_cast = (tfsmTrace.States.tfsm.TracedTimeFSM) value
							.getParent();
